...
| `paragraph-length` | Yes. `flags a paragraph over 6 sentences as a hard violation`. | Yes. `counts e.g. and i.e. within four sentences`. | Yes. `preserves a true boundary after a %s`; `does not restore a capital initial from a masked dotted identifier`. |
